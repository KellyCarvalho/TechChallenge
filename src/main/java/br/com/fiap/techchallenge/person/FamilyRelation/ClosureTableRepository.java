...
public interface ClosureTableRepository extends JpaRepository<ClosureTable, Long> {
    ...
    WITH Ancestor AS (
        ...
        SELECT ct.descendant_id, ct.depth FROM closure_table ct JOIN Ancestor a ON ct.ancestor_id = a.id WHERE ancestor_id = :descendantId
    ) SELECT p.id, p.name, p.birth_date, p.gender, d.depth FROM Descentants d JOIN person p ON d.descendant_id = p.id;
    """, nativeQuery = true)
